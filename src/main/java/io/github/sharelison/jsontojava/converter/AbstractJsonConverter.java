...
public abstract class AbstractJsonConverter implements JsonConverter {
    ...
    protected List<JsonClassResult> convertJsonToJava(String json, String objectName, String packageName,
        ...
        if (jsonValidator().isValidJson(json)) {
            ...
            return javaClasses.values().stream().map(jcb -> new JsonClassResult(jcb.getClassName(), jcb.build()))
    ...
    private JavaClassBuilder convert(Map<String, JavaClassBuilder> javaClasses, String json, String objectName,
            String packageName, boolean withAnnotations) throws JsonToJavaException {
    ...
    private JavaClassBuilder convertObject(Map<String, JavaClassBuilder> javaClasses, JSONObject jsonObject,
            String objectName, String packageName, boolean withAnnotations) {
        JavaClassBuilder javaClassBuilder = javaClasses.keySet().contains(objectName) ? javaClasses.get(objectName)
                : new JavaClassBuilder(objectName, packageName, withAnnotations);
        ...
        for (String key : jsonObject.keySet()) {
            ...
            if (value != null && !(!(value instanceof String) && "null".equals(value.toString()))) {
                ...
                if (!SinglePropertyType.NEW.equals(propertyType)) {
                    ...
                    JavaClassBuilder propertyJavaNewClass = convert(javaClasses, value.toString(),
                            JavaClassBuilder.firstCharToUpperCase(key), packageName, withAnnotations);
    ...
    private JavaClassBuilder convertArray(Map<String, JavaClassBuilder> javaClasses, JSONArray jsonArray,
            String objectName, String packageName, boolean withAnnotations) {
        JavaClassBuilder javaClassBuilder = null;
    ...
    private void addProperty(Map<String, JavaClassBuilder> javaClasses, JavaClassBuilder javaClassBuilder, String key,
            PropertyType propertyType, Object value, String packageName, boolean withAnnotations) {
        if (propertyType instanceof ComplexPropertyType) {
            ComplexPropertyType complexPropertyType = (ComplexPropertyType) propertyType;
            javaClassBuilder.addProperty(key, complexPropertyType,
                    findGenericForList(javaClasses, key, value, packageName, withAnnotations));
    ...
    protected String findGenericForList(Map<String, JavaClassBuilder> javaClasses, String key, Object value,
            String packageName, boolean withAnnotations) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (SinglePropertyType.NEW.equals(propertyType)) {
                ...
                types.add(String.format(propertyType.getDeclareName(),
                        findGenericForList(javaClasses, key, valueInList, packageName, withAnnotations)));
            } else {
